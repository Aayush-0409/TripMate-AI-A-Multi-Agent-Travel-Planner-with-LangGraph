...
def airport_country_matches(airport: dict, country_code: str) -> bool:
    ...
    airport_country = airport.get("country_code")
    if airport_country == country_code:
        return True

    try:
        country = pycountry.countries.get(alpha_2=country_code)
        if country and airport_country and airport_country.lower() == country.name.lower():
            return True
    except Exception:
        pass
...
def get_best_airport_for_country(country_code: str):
    ...
    for iata, airport in AIRPORTS.items(): # type: ignore
        if not iata:
            continue
